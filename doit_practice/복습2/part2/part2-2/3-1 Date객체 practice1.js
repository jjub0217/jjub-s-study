...
let now = new Date();
let nowMonth = now.getDate() + 50; // <- 현재 날짜로부터 50일이 지난 일수
console.log(nowMonth); // 81

// let currentMonth = now.setDate(now.getDate()); // <- 50일이 지난 날짜를 밀리초로 바꾼것.
let currentMonth = now.setDate(nowMonth); // <- 50일이 지난 날짜를 밀리초로 바꾼것.
